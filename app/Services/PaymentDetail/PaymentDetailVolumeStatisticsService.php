...
class PaymentDetailVolumeStatisticsService
{
    ...
    private function calculateModalPayload(PaymentDetail $paymentDetail, string $period): array
    {
        ...
        $volumeMoney = Money::fromUnits((string) $volumeUnits, $currencyCode);
    ...
    private function formatModalDealBucketLabel(float $min, float $max, Currency $currency): string
    {
    ...
    private function displayAmountToUnits(float $amount, string $currencyCode): int
    {
        if ($amount <= 0) {
            return 0;
        }

        $normalizedAmount = fmod($amount, 1.0) === 0.0
            ? (string) (int) round($amount)
            : rtrim(rtrim(number_format($amount, 2, '.', ''), '0'), '.');

        return Money::fromPrecision($normalizedAmount, $currencyCode)->toUnitsInt();
    }
