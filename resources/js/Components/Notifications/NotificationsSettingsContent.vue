...
const settingsSource = computed(() => usePage().props.notificationsSettings ?? {});

const telegramAccount = ref(settingsSource.value.telegramAccount ?? {});
const audioTracks = ref(settingsSource.value.audioTracks ?? []);
const notificationSoundSettings = ref(settingsSource.value.notificationSoundSettings ?? {});
const showInAppSoundSettings = ref(settingsSource.value.showInAppSoundSettings ?? false);
...
const syncFromPageProps = () => {
    ...
    telegramAccount.value = source.telegramAccount ?? {};
    audioTracks.value = source.audioTracks ?? [];
    notificationSoundSettings.value = source.notificationSoundSettings ?? {};
    showInAppSoundSettings.value = source.showInAppSoundSettings ?? false;
