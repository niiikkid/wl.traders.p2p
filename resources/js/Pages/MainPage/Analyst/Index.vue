...
const activeChartTab = ref('turnover');
...
const chartTabs = [
    { value: 'turnover', label: 'Оборот', colorToken: 'primary', seriesName: 'Оборот ($)' },
...
const applyFilter = (options = {}) => {
    ...
    router.visit(route('analyst.main.index'), {
        data: requestData,
        preserveScroll: true,
        preserveState: true,
        replace: true,
...
const loadFilterOptions = async (typeKey, query = '') => {
    ...
    try {
        const response = await axios.get(
            route('analyst.main.filter-options', { type: typeKey }),
            {
                params: {
                    query,
        ...
        selectedOptions.value[typeKey] = [
            ...
            return array.findIndex((item) => Number(item.value) === Number(option.value)) === index;
        });
    } catch (error) {
...
const getDisplayedOptions = (typeKey) => {
    ...
    const selectedIdsSet = new Set((selectedFilters.value[typeKey] || []).map((id) => Number(id)));
    const rest = (searchResults.value[typeKey] || []).filter((option) => {
        return !selectedIdsSet.has(Number(option.value));
    });
