            ...
            <div class="card bg-base-100 shadow">
                <div class="card-body">
                    <div class="flex justify-between">
                        <h3 class="card-title">Баланс агента</h3>
                        <template v-if="viewStore.isAdminViewMode">
                            <div class="join">
                                <button
                                    @click.prevent="modalStore.openWithdrawalModal({user}); setBalanceType('agent')"
                                    type="button"
                                ...
                                <button
                                    @click.prevent="modalStore.openDepositModal({user}); setBalanceType('agent')"
                                    type="button"
                        ...
                        <template v-else-if="viewStore.isAgentViewMode">
                            <button
                                @click.prevent="modalStore.openWithdrawalModal({user}); setBalanceType('agent')"
                                type="button"
                    ...
                    <div class="pt-1 inline-block align-middle">
                        <span class="text-xl font-bold">
                            {{ walletStats.totalAvailableBalances.agent.primary }} {{ primaryCurrency }}
                        </span>
                    </div>
                    ...
                    <div class="mt-0">
                        <div class="inline-flex">
                            ...
                            <div class="text-sm ml-1.5">
                                {{ walletStats.lockedForWithdrawalBalances.agent.primary }} {{ primaryCurrency }}
                            </div>
                        </div>
                    </div>
                </div>
