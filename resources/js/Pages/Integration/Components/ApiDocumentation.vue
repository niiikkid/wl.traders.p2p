    ...
    throw new Error('Invalid webhook signature');
                    ...
                    <div class="card-body space-y-4">
                        ...
                        <div class="grid gap-6">
                            ...
                            <section class="p-4 rounded-xl border border-base-200 space-y-3">
                                ...
                                <div>
                                    <h4 class="font-semibold mb-2">Ответ сервера</h4>
                                    <pre class="bg-base-200 p-4 rounded-lg overflow-x-auto text-sm"><code>{{ formatJSON({ success: true, data: [{ name: "Сбербанк", code: "sberbank", schema: "100000000111", currency: "rub", min_limit: "1000", max_limit: "100000", reservation_time: 10, detail_types: ["card", "phone", "mobile_commerce", "account_number", "iban_uah", "e-com"] }] }) }}</code></pre>
                                </div>
                            </section>
                        </div>
                    </div>
                                    ...
                                    <div class="overflow-x-auto">
                                        <table class="table table-zebra w-full">
                                            ...
                                            <tr>
                                                ...
                                                <td>тип реквизита: card, phone, mobile_commerce, account_number, iban_uah, e-com.</td>
                                            </tr>
                                            <tr>
                ...
                <article id="manual-control-acquiring" class="card bg-base-100 shadow">
                    <div class="card-body space-y-4">
                        ...
                        <p class="text-sm text-base-content/80">
                            Перед подбором реквизитов проверяется, что в панели Manual Control Acquiring есть хотя бы один оператор с включённым режимом работы.
                            Одновременно допускается до трёх сделок в статусе <code class="bg-base-200 px-1 rounded text-xs">pending</code> с этим режимом на каждого такого оператора (лимит суммируется по числу операторов в работе).
                            Если операторов нет или лимит исчерпан, создание отклоняется с отдельной ошибкой в каждом случае.
                        </p>
