...
const confirmAcceptOrder = (order) => {
    modalStore.openConfirmModal({
        ...
        confirm: () => {
            useForm({}).patch(route('analyst.orders.accept', order.id), {
                preserveScroll: true,
                onSuccess: () => {
                    modalStore.closeAll()
                    router.visit(route('analyst.orders.index'), {
                        only: ['orders'],
                    })
                },
            })
...
const confirmAcceptDispute = (dispute) => {
    modalStore.openConfirmModal({
        ...
        confirm: () => {
            useForm({}).patch(route('analyst.disputes.accept', dispute.id), {
                preserveScroll: true,
                onFinish: () => {
                    modalStore.closeAll()
                    router.visit(route('analyst.orders.index'), {
...
const confirmRollbackDispute = (dispute) => {
    modalStore.openConfirmModal({
        ...
        confirm: () => {
            useForm({}).patch(route('analyst.disputes.rollback', dispute.id), {
                preserveScroll: true,
                onFinish: () => {
                    modalStore.closeAll()
                    router.visit(route('analyst.orders.index'), {
...
const openManualControlAcqPage = () => {
    ...
    window.open(route('analyst.manual-control-acq.show'), '_blank', 'noopener');
};
