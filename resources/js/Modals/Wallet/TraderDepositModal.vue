...
const show = computed(() => modalStore.isOpen('traderDeposit'));
...
const close = () => {
    modalStore.close('traderDeposit');
    amount.value = '';
    error.value = '';
};
...
<template>
    <Modal :show="show" size="sm" @close="close">
        ...
        <ModalBody>
            <div>
                <InputLabel for="trader_deposit_amount" value="Сумма" :error="!!error" />
                <TextInput
                    id="trader_deposit_amount"
                    v-model="amount"
                    type="number"
                    step="0.01"
                    min="0"
