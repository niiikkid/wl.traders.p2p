...
<div class="content"><header class="intro"><div class="intro-row"><div><span class="kicker">Шаг 1 · Заполните настройки</span><h2>Установка WL Traders<br><span>на ваш Ubuntu‑сервер.</span></h2></div><span class="system-tag">Готов к настройке</span></div></header>
<form id="form">
...
<section class="launch"><div class="launch-panel"><div class="launch-copy"><span class="kicker">Последний шаг</span><h3>Проверьте настройки и начните установку.</h3><p>Установщик подготовит сервер, создаст базу данных, соберёт интерфейс и запустит WL Traders. Процесс и возможные ошибки будут показаны ниже.</p></div><div class="actions"><span class="warning">Установка остановится, если выбранная база уже содержит данные. Ничего существующего удаляться не будет.</span><button class="button" type="submit">Начать установку →</button></div></div></section>
</form>
<section id="status" class="status" aria-live="polite"><div class="status-head"><div class="pill"><span class="dot"></span><b id="message">Подготовка…</b></div><span class="status-label">Ход установки</span></div><div class="bar"><i></i></div><pre id="logs"></pre></section>
...
document.querySelectorAll('.reveal-secret').forEach(button=>button.addEventListener('click',()=>{const input=button.previousElementSibling;const show=input.type==='password';input.type=show?'text':'password';button.textContent=show?'Скрыть':'Показать';button.setAttribute('aria-label',show?'Скрыть пароль':'Показать пароль')}));
