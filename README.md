...
- **домен** — установщик проверит DNS и откроет продукт по домену через HTTP;
- **IP-адрес** — продукт будет доступен по HTTP.

Для домена создайте DNS-запись типа `A`, направленную на IP сервера. После установки продукт будет работать по HTTP. Чтобы получить SSL и защитить сервер, можно подключить Cloudflare: включите **Proxied** (оранжевое облако), выберите режим **SSL/TLS → Full (strict)** и включите **Always Use HTTPS**. Cloudflare будет принимать веб-трафик перед сервером, шифровать соединение и помогать фильтровать вредные запросы.
