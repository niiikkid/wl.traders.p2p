...
Откройте её со своего компьютера. Для обычной установки достаточно указать адрес сайта и пароль администратора; безопасные значения для базы, firewall, swap и бэкапов уже выбраны. Дополнительные ключи Telegram, TronGrid и IP Geolocation можно оставить пустыми.
