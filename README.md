...
**1. На своём компьютере, НЕ в консоли сервера:** откройте новое окно Терминала (Mac/Linux) или PowerShell (Windows) и выполните:

```bash
ssh -o ExitOnForwardFailure=yes -L 8787:127.0.0.1:8787 root@IP_СЕРВЕРА
```

Вместо `IP_СЕРВЕРА` укажите адрес из панели хостера. Этот способ защищает ввод пароля и ключей установщика через SSH. **Не выполняйте эту команду в веб-консоли хостинга или внутри уже открытого SSH-подключения** — сервер подключится к самому себе и займёт порт мастера.

**2. Теперь на сервере:** в подключении, открытом на шаге 1, выполните:
